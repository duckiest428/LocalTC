...
from localtc.sim_api import AtcTransmission, AtisBroadcast, RadioTuned, Transcript
from localtc.tts.player import Clip
from localtc.tts.voices import PILOT_SPEAKER_SALT, SPEAKERS, speaker_for
...
class VoiceOut:
    def __init__(
        ...
        *,
        ...
        self._events = bus.subscribe(AtcTransmission, AtisBroadcast, RadioTuned, Transcript)
        ...
        try:
            async for ev in self._events:
                if isinstance(ev, AtcTransmission):
                    await self.say(ev.spoken or ev.text, ev.station, "atc")
    ...
    async def say(self, text: str, voice_key: str, kind: str) -> Clip | None:
        async with self._lock:
            clip = await asyncio.to_thread(self.render, text, voice_key, kind)
            if clip is not None:
                self.player.play(clip)
            return clip

    def render(self, text: str, voice_key: str, kind: str) -> Clip | None:
